...
async function fetchMe(config: any): Promise<UserProfile> {
  const res = await axios.get<UserProfile>(`http://localhost:5000/me`, config)
  return res.data
}

async function updateMe(payload: Partial<UserProfile>, config: any): Promise<UserProfile> {
  const res = await axios.patch<UserProfile>(`http://localhost:5000/me`, payload, config)
  return res.data
}

async function uploadAvatar(file: File, config: any): Promise<{ avatarUrl: string }> {
  const form = new FormData()
  form.append('avatar', file)
  const res = await axios.post<{ avatarUrl: string }>(`http://localhost:5000/me/avatar`, form, {
    ...config,
    headers: {
      ...config.headers,
...
async function changePassword(payload: { currentPassword: string; newPassword: string }, config: any) {
...
function ProfileContent() {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: Partial<UserProfile>) => updateMe(payload, authConfig),
    onSuccess: (updated) => {
    ...
    onError: (err: any) => {
      const message =
          (axios.isAxiosError(err) && ((err.response?.data as any)?.message || err.message)) ||
  ...
  const avatarMutation = useMutation({
    mutationFn: (file: File) => uploadAvatar(file, authConfig),
    onSuccess: ({ avatarUrl }) => {
      enqueueSnackbar('Avatar updated', { variant: 'success' })
      queryClient.setQueryData<UserProfile | undefined>(['me'], (prev) =>
  ...
  const passwordMutation = useMutation({
    mutationFn: (payload: { currentPassword: string; newPassword: string }) =>
        changePassword(payload, authConfig),
              ...
              <Stack spacing={2}>
                ...
                >
                  ...
                  <Switch
                      ...
                      onChange={(_, checked) => setValue('emailNotifications', checked, { shouldDirty: true })}
                  />
                </Stack>
